...
router.post('/users', async (req, res) => {
    ...
    try {
        ...
        res.status(201).render('usermade', {
            user: user,
            token: token
        })
        ...
        if (e.errmsg){
            res.status(400).render('usercreation', {
                error: 'Account under this email already exists. Please try again.',
                name: 'Philip Clement'
            })
        }
        else {
            res.status(400).render('usercreation',{
                error: "Please enter a valid password (7 or longer)"
            })
        }
    }
...
router.get('/users/me/read', auth, async (req, res) => {
    res.send(req.user)
})
...
router.post('/users/me/avatar/upload', auth, upload.single('avatar'), async (req,res)=>{
